...
export class Scene extends BaseScene {
  ...
  preload() {
    ...
    // Load as spritesheet (because it contains multiple frames)
      ...
      (_, star) => {
        ...
        if (stars.countActive(true) === 0) {
          stars.children.iterate((star) => {
            (star as Phaser.Physics.Arcade.Image).enableBody(
              true,
              // @ts-expect-error Property 'x' does not exist on type 'GameObject'
              star.x,
              0,
              true,
              true,
